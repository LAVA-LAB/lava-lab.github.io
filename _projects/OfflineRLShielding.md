...
**Background:** In the offline reinforcement learning (RL) problem, we aim to train/optimise an agent to perform a task from a fixed dataset of experiences. The problem is that no additional data can be collected, so the outcome of untested actions is unclear. Therefore, how to effectively evaluate the agent, so-called off-policy evaluation, remains an open problem. It is therefore hard to guarantee that the agent will perform well, and equally important, will only take safe actions. In previous work, the safety of RL agents has been improved through so-called shields, that only allows the agent to take safe actions.
